...
async function ManggaToon3(judul) {
	try {
...
async function Otakudesu(query) {
	try {
	const link = await axios.get(`https://otakudesu.video/?s=${query}&post_type=anime`)
	const c = cheerio.load(link.data)
...
async function FilmApik23(query) {
	try{
...
async function GSMArena(query) {
	const link = await axios.get(`https://www.gsmarena.com/res.php3?sSearch=${query}`)
...
async function zodiakMing(query) {
	const link = await axios.get(`https://www.fimela.com/zodiak/${query}/minggu-ini`)
	const  $ = cheerio.load(link.data)
...
async function zodiakHar (query) {
	let Hasil = []
	await axios.request(`https://www.fimela.com/zodiak/${query}`, {
			method: "GET",
			headers: {
				"accept": "text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.9",
				"accept-language": "en-US,en;q=0.9,id;q=0.8",
...
module.exports.ManggaToon3 = ManggaToon3
